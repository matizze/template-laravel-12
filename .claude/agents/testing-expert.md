...
- Test architecture from **architect**
- Test models from **eloquent-specialist**
- Test APIs from **api-developer**
- Follow security tests from **security-auditor**
- Verify performance with **performance-optimizer**
...
### Setup
- Dusk usa `.env.dusk.local` para configuração — sempre verifique se APP_URL corresponde ao servidor real
- Se usando **Laravel Herd**, APP_URL deve ser o domínio do Herd (ex: `http://template-laravel-12.test`), NÃO `127.0.0.1`
- Se usando `composer dev` / `php artisan serve`, use `http://127.0.0.1:8000` (ou a porta configurada)
- O servidor da aplicação DEVE estar rodando antes dos testes Dusk
- Rodar testes: `php artisan dusk` ou `php artisan dusk --filter=TestName`
- Testes Dusk ficam em `tests/Browser/` e estendem `Tests\DuskTestCase`
- Use `DatabaseMigrations` trait (NÃO `RefreshDatabase`) em testes Dusk

### Erros Comuns e Soluções

#### ERR_CONNECTION_REFUSED
- **Causa**: APP_URL em `.env.dusk.local` não corresponde ao servidor rodando
- **Fix**: Verifique se APP_URL bate com o dev server. Se usando Herd, use o domínio do Herd. Se usando `composer dev`, use `http://127.0.0.1:8000`
- **Diagnóstico**: Rode `curl -I <APP_URL>` para verificar se o servidor responde antes de executar o Dusk

#### Seletor Não Encontrado / Elemento Não Interagível
- **Causa**: Seletores Dusk (`@dusk-selector`) devem existir no HTML renderizado
- **Fix**: Sempre verifique se os seletores existem nos templates Blade antes de escrever os testes. Use atributo `dusk="nome-do-seletor"` nos elementos HTML
- **Dica**: Ao testar modais ou dropdowns, clique no trigger primeiro, depois use `waitFor` para o conteúdo aparecer antes de interagir

#### Padrão de Interação com Modal/Dropdown
```php
$browser->click('@botao-trigger')
    ->waitFor('@conteudo-dropdown', 5)
    ->click('@item-dropdown');
```

#### Imports Não Utilizados Após Escrever Testes
- Após escrever testes Dusk, sempre verifique imports não utilizados (Pint vai flagar)
- Importe apenas models/classes que são realmente usados no corpo do arquivo de teste

### Boas Práticas Dusk
- Use `waitForText()` e `waitFor()` ao invés de `assertSee()` para conteúdo dinâmico — Dusk precisa de tempo para carregamento de página
- Use `waitForLocation()` após submissões de formulário que redirecionam
- Sempre adicione waits explícitos com timeout para elementos assíncronos: `->waitFor('@elemento', 5)`
- Teste fluxos de usuário end-to-end: login → navegar → interagir → verificar resultado
- Use o padrão `$this->browse(function (Browser $browser) { ... })`
- Para testes de formulário, verifique TANTO feedback visual QUANTO estado do banco com `assertDatabaseHas()`
- Mantenha testes Dusk focados em fluxos de UI — use Feature tests para lógica de negócio

### Estrutura de Teste Dusk
```php
class ExemploFluxoTest extends DuskTestCase
{
    use DatabaseMigrations;

    public function test_usuario_completa_fluxo(): void
    {
        ...
        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user)
                ->visit('/pagina-alvo')
                ->waitForText('Conteúdo Esperado')
                ->type('#campo', 'valor')
                ->press('Enviar')
                ->waitForText('Sucesso')
                ->assertPathIs('/redirecionamento-esperado');
        });

        $this->assertDatabaseHas('tabela', ['coluna' => 'valor']);
    }
}
```

### Dusk em Worktrees
- Se trabalhando em git worktree, garanta que o worktree tenha seu próprio `.env.dusk.local` com APP_URL correto
- O servidor deve estar rodando no contexto do worktree ou Herd deve estar configurado para servir o path do worktree
- ChromeDriver deve estar instalado: `php artisan dusk:chrome-driver`

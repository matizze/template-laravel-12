                    ...
                    <tr class="border-t border-dotted border-gray-200">
                        ...
                        <td class="px-4 py-4">
                            <span @class([
                                ...
                                'bg-indigo-100 text-indigo-700' => $membership->role === \Modules\Workspace\Enums\WorkspaceRole::Owner,
                                'bg-blue-100 text-blue-700' => $membership->role === \Modules\Workspace\Enums\WorkspaceRole::Admin,
                                'bg-gray-100 text-gray-700' => $membership->role === \Modules\Workspace\Enums\WorkspaceRole::Member,
                                'bg-gray-50 text-gray-500' => $membership->role === \Modules\Workspace\Enums\WorkspaceRole::Viewer,
                            ])>
                        ...
                        <td class="flex gap-2 px-4 py-4">
                            @can('manageMembers', $workspace)
                                @if($membership->role !== \Modules\Workspace\Enums\WorkspaceRole::Owner)
                                    <form method="POST" action="{{ route('workspace.members.updateRole', [$workspace, $membership->user]) }}">
                                        ...
                                        >
                                            @foreach (\Modules\Workspace\Enums\WorkspaceRole::cases() as $role)
                                                @if($role !== \Modules\Workspace\Enums\WorkspaceRole::Owner)
